...
// added by alipaymate@qq.com
if ($_SESSION['payment'] == 'alipay_forcard') {
  // delete if order existed
  if (isset($_SESSION['alipay_forcard_pending_order_id'])) {
      $_order_id = $_SESSION['alipay_forcard_pending_order_id'];

      $_order_query = 'select orders_id from ' . TABLE_ORDERS . " where orders_status = " . DEFAULT_ORDERS_STATUS_ID . " and payment_module_code = 'alipay_forcard' and orders_id = " . (int)$_order_id;
      $_orders = $db->Execute($_order_query);

      if ($_orders->RecordCount() > 0) {
         $db->Execute('delete from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = ' . $_order_id);
         $db->Execute('delete from ' . TABLE_ORDERS_TOTAL          . ' where orders_id = ' . $_order_id);
         $db->Execute('delete from ' . TABLE_ORDERS                . ' where orders_status = 1 and orders_id = ' . $_order_id);
      }
  ...
  $cart_type = '';
  if (isset($_SESSION['alipay_card_type'])) {
      $cart_type = $_SESSION['alipay_card_type'];
  }


  $order->info['payment_method'] = MODULE_PAYMENT_ALIPAYFOR_TEXT_TITLE . ': ' . $cart_type;
  $order->info['payment_module_code'] = 'alipay_forcard';
  ...
  $_SESSION['alipay_forcard_pending_order_id'] = $insert_id;
}
?>