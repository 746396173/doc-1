...
if ($_SESSION['payment'] == 'alipay_forcard') {
    if (isset($_SESSION['alipay_forcard_success_order_id'])) {
        $_order_id = (int)$_SESSION['alipay_forcard_success_order_id'];

        $db->Execute('update ' . TABLE_ORDERS . ' set last_modified=now(), orders_status = ' . MODULE_PAYMENT_ALIPAYFOR_ORDER_STATUS_ID . ' where orders_id = ' . $_order_id);
        $sql_data_array = array('orders_id'         => $_order_id,
